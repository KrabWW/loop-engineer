...
source_finisher=$(cd "$test_dir/.." && pwd -P)/finish-omc-task
tmp_root=$(mktemp -d "${TMPDIR:-/tmp}/finish-omc-task.XXXXXX")
...
set -euo pipefail
printf '%s\n' "$*" >> "${FAKE_OMC_LOG:?}"
[ -n "${OMC_STATE_DIR:-}" ] || { printf "missing OMC_STATE_DIR\n" >&2; exit 8; }
[ -n "${OMC_RUNTIME_V2:-}" ] || { printf "missing OMC_RUNTIME_V2\n" >&2; exit 8; }
[ "${1:-}" = team ] || exit 2
sub=${2:-}
case "$sub" in
  api)
    ...
    [ "$name" = "random-team-7f3a" ] || { printf "wrong team: %s\n" "$name" >&2; exit 9; }
    case "$op" in
      get-summary)
        case "${FAKE_OMC_MODE:-complete}" in
          active) in_prog=1; comp=0; failed=0 ;;
          failed) in_prog=0; comp=0; failed=1 ;;
          *) in_prog=0; comp=1; failed=0 ;;
        esac
        printf '{"ok":true,"data":{"summary":{"teamName":"%s","workerCount":1,"tasks":{"total":1,"pending":0,"blocked":0,"in_progress":%s,"completed":%s,"failed":%s},"workers":[{"name":"worker-1","alive":true}],"nonReportingWorkers":[]}}}\n' "$name" "$in_prog" "$comp" "$failed"
        ;;
      read-worker-status) printf '{"ok":true,"data":{"worker":"worker-1","status":{"state":"idle","updated_at":"2026-07-17T00:00:00.000Z"}}}\n' ;;
      read-worker-heartbeat)
        if [ "${FAKE_OMC_MODE:-complete}" = stale-heartbeat ]; then
          printf '{"ok":true,"data":{"worker":"worker-1","heartbeat":{"ts":1}}}\n'
        else
    ...
    ;;
  shutdown)
    [ "${FAKE_OMC_MODE:-complete}" != shutdown-fail ] || { printf "shutdown failed\n" >&2; exit 17; }
    if [ "${FAKE_OMC_MODE:-complete}" = shutdown-content-change ]; then
      ...
      git commit --allow-empty -qm "simulate OMC shutdown checkpoint"
    fi
    if [ "${FAKE_OMC_MODE:-complete}" = main-drift ]; then
    ...
    if [ "${FAKE_OMC_MODE:-complete}" != shutdown-incomplete ]; then
      rm -rf "$OMC_STATE_DIR"/fake-project/state/team/random-team-7f3a 2>/dev/null || true
      git branch -D omc-team/random-team-7f3a/worker-1 2>/dev/null || true
    fi
...
printf '%s\n' "$*" >> "${FAKE_TMUX_LOG:?}"
case "${1:-}" in
  has-session) shift; [ "${1:-}" = -t ] && shift; [ -f "$state/session-${1:?}" ] ;;
  display-message)
    shift; [ "${1:-}" = -p ] && shift; [ "${1:-}" = -t ] && shift; pane=${1:?}; shift
    [ "${1:-}" = "#{pane_dead}" ] || exit 2
    case "${FAKE_TMUX_MODE:-ok}" in
      ok|kill-stuck) [ "$pane" = %42 ] && [ -f "$state/pane-%42" ] ;;
  ...
  list-panes)
    ...
    case "${FAKE_TMUX_MODE:-ok}" in
      ok|kill-stuck) printf '%%42\t0\t%s\n' "$worktree" ;;
      replaced-pane) printf '%%77\t0\t%s\n' "$worktree" ;;
      reused-pane) printf '%%42\t0\t%s/.omc/team/foreign\n%%77\t0\t%s\n' "$worktree" "$worktree" ;;
      multiple-panes) printf '%%77\t0\t%s\n%%78\t0\t%s\n' "$worktree" "$worktree" ;;
      missing-pane) printf '%%77\t0\t%s/.omc/team/worker-1\n' "$worktree" ;;
      *) exit 2 ;;
    esac ;;
  kill-session)
...
write_task() {
  ...
  mkdir -p "$(dirname "$path")"
  {
    printf '%s\n' \
      ...
      "- Status: \`${status}\`" '- Owner: `docs/contracts/sample`' '- Depends on: `none`' '' \
      '## Goal' '' 'Freeze one sample contract.' '' '## Allowed Files' '' \
      '- `docs/contracts/sample/README.md`' '- `docs/tasks/contracts/PROTO-SAMPLE-001.md`'
...
state_base_for() { git -C "$1" rev-parse --path-format=absolute --git-common-dir; }

write_launch_state() {
  wt=$1; repo=$2; sbase=$3
  fingerprint=$(reference_fingerprint "$repo/refer")
  mkdir -p "$sbase"
  printf '{"task_id":"PROTO-SAMPLE-001","task_file":"docs/tasks/contracts/PROTO-SAMPLE-001.md","branch":"codex/omc-qs-proto-sample-001","worktree":"%s","session":"omc-qs-proto-sample-001","state_base":"%s","derived_ready":false,"refer_fingerprint_version":"content-v2","refer_fingerprint":"%s","leader_pane":"%%42","team_name":"random-team-7f3a","omc_version":"fake omc 0.0"}\n' \
    "$wt" "$sbase" "$fingerprint" > "$sbase/launch.json"
}

refresh_evidence() {
  wt=$1
  mkdir -p "$wt/.omc/context"
  head=$(git -C "$wt" rev-parse HEAD)
  printf '# final evidence\n\n- Final HEAD: `%s`\n- Verification: PASS\n' "$head" > "$wt/.omc/context/proto-sample-001-final-evidence.md"
}

seed_team_state() {
  sbase=$1
  team_dir="$sbase/fake-project/state/team/random-team-7f3a"
...
make_fixture() {
  ...
  wt="$wt_root/omc-proto-sample-001"
  mkdir -p "$repo"
  ...
  cp "$source_finisher" "$repo/scripts/finish-omc-task"
  chmod +x "$repo/scripts/finish-omc-task"
  ...
  mkdir -p "$wt_root"
  git -C "$repo" worktree add -q -b codex/omc-qs-proto-sample-001 "$wt" main
  ...
  git -C "$wt" switch -q codex/omc-qs-proto-sample-001
  ...
  sbase="$(state_base_for "$repo")/omc-task-state/proto-sample-001"
  seed_team_state "$sbase"
  git -C "$wt" branch "omc-team/random-team-7f3a/worker-1" HEAD
  mkdir -p "$tmp_root/tmux-$name"
  : > "$tmp_root/tmux-$name/session-omc-qs-proto-sample-001"
  : > "$tmp_root/tmux-$name/pane-%42"
  : > "$tmp_root/tmux-$name/session-omc-qs-foreign-001"
  foreign_base="$(state_base_for "$repo")/omc-task-state/foreign-slug"
  mkdir -p "$foreign_base/fake-project/state/team/foreign-team/workers/worker-9"
  write_launch_state "$wt" "$repo" "$sbase"
  refresh_evidence "$wt"
  printf '%s\n' "$repo"
}
...
run_finisher() {
  repo=$1; shift
  name=$(basename "$repo")
  task_worktree=$(cd "${repo}-task-worktrees/omc-proto-sample-001" && pwd -P)
  : > "$tmp_root/omc-$name.log"
  : > "$tmp_root/tmux-$name.log"
  (
    cd "$repo"
    FAKE_OMC_LOG="$tmp_root/omc-$name.log" \
      FAKE_TMUX_LOG="$tmp_root/tmux-$name.log" \
      FAKE_TMUX_STATE="$tmp_root/tmux-$name" \
      FAKE_TASK_WORKTREE="$task_worktree" \
      FAKE_OMC_MODE="${FAKE_OMC_MODE:-complete}" \
      ...
      OMC_TASK_WORKTREE_ROOT="${repo}-task-worktrees" \
...
test ! -e "${repo}-task-worktrees/omc-proto-sample-001" || fail 'success left worktree'
git -C "$repo" show-ref --verify --quiet refs/heads/codex/omc-qs-proto-sample-001 && fail 'success left branch'
test ! -f "$tmp_root/tmux-success/session-omc-qs-proto-sample-001" || fail 'success left tmux session'
test -f "$tmp_root/tmux-success/session-omc-qs-foreign-001" || fail 'success touched foreign tmux session'
foreign_base="$(state_base_for "$repo")/omc-task-state/foreign-slug"
test -d "$foreign_base/fake-project/state/team/foreign-team/workers" || fail 'success touched foreign Team state'
...
sbase="$(state_base_for "$repo")/omc-task-state/proto-sample-001"
write_launch_state "${repo}-task-worktrees/omc-proto-sample-001" "$repo" "$sbase"
...
assert_contains "$output" 'no live exact-cwd leader pane in omc-qs-proto-sample-001'
assert_not_contains "$(<"$tmp_root/omc-missing-pane.log")" 'shutdown'
...
assert_contains "$output" 'multiple live exact-cwd leader panes in omc-qs-proto-sample-001'
assert_not_contains "$(<"$tmp_root/omc-multiple-panes.log")" 'shutdown'

# A second team under the same state base must not interfere: finisher targets
# the launch-record team (random-team-7f3a) and ignores the other.
repo=$(make_fixture multiple-teams)
sbase="$(state_base_for "$repo")/omc-task-state/proto-sample-001"
mkdir -p "$sbase/fake-project/state/team/second-team/workers/worker-2"
: > "$sbase/fake-project/state/team/second-team/workers/worker-2/.ready"
output=$(run_finisher "$repo" PROTO-SAMPLE-001)
assert_contains "$output" 'mode=finished'
assert_contains "$output" 'team_name=random-team-7f3a'
test -d "$sbase/fake-project/state/team/second-team/workers" || fail 'finisher touched the unrelated second team'

repo=$(make_fixture no-team)
sbase="$(state_base_for "$repo")/omc-task-state/proto-sample-001"
rm -rf "$sbase/fake-project/state/team/random-team-7f3a"
if output=$(run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'missing team unexpectedly finished'; fi
assert_contains "$output" 'found none'
assert_not_contains "$(<"$tmp_root/omc-no-team.log")" 'shutdown'

repo=$(make_fixture active-team)
if output=$(FAKE_OMC_MODE=active run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'active team unexpectedly finished'; fi
assert_contains "$output" 'team is not complete'
assert_not_contains "$(<"$tmp_root/omc-active-team.log")" 'shutdown'

repo=$(make_fixture stale-heartbeat)
if output=$(FAKE_OMC_MODE=stale-heartbeat run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'stale heartbeat unexpectedly finished'; fi
assert_contains "$output" 'heartbeat is stale at finish'
assert_not_contains "$(<"$tmp_root/omc-stale-heartbeat.log")" 'shutdown'

repo=$(make_fixture missing-evidence)
rm -f "${repo}-task-worktrees/omc-proto-sample-001/.omc/context/"*final-evidence*.md
...
assert_not_contains "$(<"$tmp_root/omc-missing-evidence.log")" 'shutdown'

repo=$(make_fixture stale-evidence)
wt="${repo}-task-worktrees/omc-proto-sample-001"
stale_head=$(git -C "$repo" rev-parse HEAD)
printf '# final evidence\n\n- Final HEAD: `%s`\n' "$stale_head" > "$wt/.omc/context/proto-sample-001-final-evidence.md"
...
assert_not_contains "$(<"$tmp_root/omc-stale-evidence.log")" 'shutdown'

repo=$(make_fixture dirty-worktree)
printf 'dirty\n' > "${repo}-task-worktrees/omc-proto-sample-001/untracked.txt"
...
assert_not_contains "$(<"$tmp_root/omc-dirty-worktree.log")" 'shutdown'
...
assert_not_contains "$(<"$tmp_root/omc-scope-violation.log")" 'shutdown'
...
assert_not_contains "$(<"$tmp_root/omc-protected-allowed.log")" 'shutdown'

repo=$(make_fixture refer-mutated)
printf 'mutated\n' >> "$repo/refer/reference.txt"
if output=$(run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'mutated refer baseline unexpectedly finished'; fi
assert_contains "$output" 'refer/ baseline changed'
assert_not_contains "$(<"$tmp_root/omc-refer-mutated.log")" 'shutdown'
...
assert_not_contains "$(<"$tmp_root/omc-verification-failure.log")" 'shutdown'

repo=$(make_fixture shutdown-failure)
if output=$(FAKE_OMC_MODE=shutdown-fail run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'shutdown failure unexpectedly finished'; fi
assert_contains "$output" 'team shutdown failed'
test -f "$tmp_root/tmux-shutdown-failure/session-omc-qs-proto-sample-001" || fail 'shutdown failure removed recovery session'

repo=$(make_fixture shutdown-content-change)
if output=$(FAKE_OMC_MODE=shutdown-content-change run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'shutdown content change unexpectedly finished'; fi
assert_contains "$output" 'shutdown changed repository content'
test -f "$tmp_root/tmux-shutdown-content-change/session-omc-qs-proto-sample-001" || fail 'content change removed recovery session'

# Shutdown-completeness gate: omc team shutdown exit 0 but team state + worker
# branch survive -> finisher must fail closed and retain recovery resources.
repo=$(make_fixture shutdown-incomplete)
if output=$(FAKE_OMC_MODE=shutdown-incomplete run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'shutdown-incomplete unexpectedly finished'; fi
assert_contains "$output" 'shutdown_incomplete'
test -d "${repo}-task-worktrees/omc-proto-sample-001" || fail 'shutdown-incomplete removed recovery worktree'
git -C "$repo" show-ref --verify --quiet refs/heads/codex/omc-qs-proto-sample-001 || fail 'shutdown-incomplete removed recovery branch'
output=$(FAKE_OMC_MODE=complete run_finisher "$repo" PROTO-SAMPLE-001)
assert_contains "$output" 'mode=finished'
...
test -d "${repo}-task-worktrees/omc-proto-sample-001" || fail 'stuck tmux removed recovery worktree'
...
test -d "${repo}-task-worktrees/omc-proto-sample-001" || fail 'rebase conflict removed recovery worktree'
...
test -d "${repo}-task-worktrees/omc-proto-sample-001" || fail 'dirty post-rebase removed recovery worktree'
test -n "$(git -C "${repo}-task-worktrees/omc-proto-sample-001" status --porcelain)" || fail 'dirty post-rebase fixture did not dirty worktree'
rm -f "${repo}-task-worktrees/omc-proto-sample-001/verification-dirty.txt"
...
test -d "${repo}-task-worktrees/omc-proto-sample-001" || fail 'dirty post-merge removed recovery worktree'
git -C "$repo" show-ref --verify --quiet refs/heads/codex/omc-qs-proto-sample-001 || fail 'dirty post-merge removed recovery branch'
...
if output=$(FAKE_OMC_MODE=main-drift run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'main drift unexpectedly finished'; fi
assert_contains "$output" 'main advanced during finish'
test -d "${repo}-task-worktrees/omc-proto-sample-001" || fail 'main drift removed recovery worktree'
output=$(FAKE_OMC_MODE=complete run_finisher "$repo" PROTO-SAMPLE-001)
assert_contains "$output" 'mode=finished'
assert_contains "$output" 'resumed=true'

repo=$(make_fixture locked)
mkdir "$repo/.git/finish-omc-task.lock"
if output=$(run_finisher "$repo" PROTO-SAMPLE-001 2>&1); then fail 'locked finisher unexpectedly ran'; fi
assert_contains "$output" 'another finish-omc-task is active or stale'
assert_not_contains "$(<"$tmp_root/omc-locked.log")" 'shutdown'

printf 'PASS finish-omc-task discovery gates pane recovery verification shutdown rebase merge cleanup recovery heartbeat isolation\n'
